...
void InputConvertNormal::mouseEvent(const QMouseEvent* from, const QSize& frameSize,
  ...
  QPoint position = from->position().toPoint();
  position.setX(position.x() * frameSize.width() / widgetSize.width());
  position.setY(position.y() * frameSize.height() / widgetSize.height());

  ControlEvent* event = new ControlEvent(ControlEventType::CET_MOUSE);
  if (!event) {
    return;
  }

  event->setMouseEventData(action, convertMouseButtons(from->buttons()),
                           QRect(position, frameSize));
...
void InputConvertNormal::wheelEvent(const QWheelEvent* from, const QSize& frameSize,
  ...
  quint32 hScroll = from->angleDelta().x() / 120;  // 每120单位为一档
  quint32 vScroll = from->angleDelta().y() / 120;

  QPoint position = from->position().toPoint();
  position.setX(position.x() * frameSize.width() / widgetSize.width());
  position.setY(position.y() * frameSize.height() / widgetSize.height());

  ControlEvent* event = new ControlEvent(ControlEventType::CET_SCROLL);
  if (!event) {
    return;
  }

  event->setScrollEventData(QRect(position, frameSize), hScroll, vScroll);
  sendControlEvent(event);
}
...
void InputConvertNormal::keyEvent(const QKeyEvent* from, const QSize& frameSize,
  ...
  ControlEvent* event = new ControlEvent(ControlEventType::CET_KEYCODE);
  if (!event) {
    return;
  }

  event->setKeycodeEventData(action, keyCode, convertMetastate(from->modifiers()));
  sendControlEvent(event);
}

AndroidMotioneventButtons InputConvertNormal::convertMouseButtons(Qt::MouseButtons buttons) {
  quint32 result = 0;
  ...
  return (AndroidMotioneventButtons)result;
}
