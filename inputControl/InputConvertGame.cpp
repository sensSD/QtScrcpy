...
void InputConvertGame::mouseEvent(const QMouseEvent* from, const QSize& frameSize,
...
void InputConvertGame::wheelEvent(const QWheelEvent* from, const QSize& frameSize,
                                  const QSize& showSize) {
  if (m_gameMap) {
    updateSize(frameSize, showSize);
  } else {
...
void InputConvertGame::keyEvent(const QKeyEvent* from, const QSize& frameSize,
                                const QSize& showSize) {
  switch (from->key()) {
...
void InputConvertGame::sendTouchEvent(int id, QPointF pos, AndroidMotioneventAction action) {
  if (0 > id || MULTI_TOUCH_MAX_NUM - 1 < id) return;

  ControlEvent* event = new ControlEvent(CET_TOUCH);
  if (!event) return;

  event->setTouchEventData(id, action, QRect(calcFrameAbsolutePos(pos).toPoint(), m_frameSize));
  sendControlEvent(event);
}

QPointF InputConvertGame::calcFrameAbsolutePos(QPointF relativePos) {
  QPointF absolutePos = relativePos;
...
int InputConvertGame::updateSteerWheelKeysPress(const QKeyEvent* from, int& keyPress1,
                                                int& keyPress2) {
  bool keyPress = false;
  if (QEvent::KeyPress == from->type()) {
    keyPress = true;
  ...
  if (from->key() == m_steerWheelKeys[SWD_DOWN]) {
    m_steerWheelKeysPress[SWD_DOWN] = keyPress;
  }
  if (from->key() == m_steerWheelKeys[SWD_LEFT]) {
    m_steerWheelKeysPress[SWD_LEFT] = keyPress;
  }
  if (from->key() == m_steerWheelKeys[SWD_RIGHT]) {
...
bool InputConvertGame::processMouseMove(const QMouseEvent* from) {
  ...
  if (!m_mouseMoveLastPos.isNull()) {
    QPointF distance = from->position() - m_mouseMoveLastPos;
    distance /= 10;
    ...
    m_mouseMoveLastConverPos.setY(m_mouseMoveLastConverPos.y() +
                                  distance.y() / m_showSize.height());

    if (m_mouseMoveLastConverPos.x() < 0.1 || m_mouseMoveLastConverPos.x() > 0.9 ||
        m_mouseMoveLastConverPos.y() < 0.1 || m_mouseMoveLastConverPos.y() > 0.9) {
      mouseMoveStopTouch();
      mouseMoveStartTouch(from);
    }
  ...
  m_mouseMoveLastPos = from->position();
  return true;
}

void InputConvertGame::moveCursorToStart(const QMouseEvent* from) {
  QPoint localPos = QPoint(m_showSize.width() * m_mouseMoveStartPos.x(),
                           m_showSize.height() * m_mouseMoveStartPos.y());
  QPoint posOffset = from->position().toPoint() - localPos;
...
void InputConvertGame::moveCursorTo(const QMouseEvent* from, const QPoint& pos) {
  QPoint posOffset = from->position().toPoint() - pos;
  QPoint globalPos = from->globalPosition().toPoint();
